...
#include "GameLoop.h"
#include "../Gameobject/Character/Dragon/Projectile/Projectile.h"
...
void projectileThing(Map map,
    ...
    auto newProjectile = std::make_shared<Projectile>(nextKnightPos.first, nextKnightPos.second,
...
void GameLoop::continueGame(Map map) {
    ...
    while (true) {
        for (int i = 0; i < map.projectiles.size(); i++) {
            if (map.projectiles[i] != nullptr) {
                ...
                switch (flag) {
                    case CanMove:
                        map.mapObjects[projectileX][projectileY].swap(
                                map.mapObjects[collisionObjectPost.first][collisionObjectPost.second]);
        ...
        for (int i = 0; i < map.zombies.size(); i++) {
            if (map.zombies[i] != nullptr) {
                ...
                switch (flag) {
                    case CanMove:
                        map.mapObjects[zombiePos.first][zombiePos.second].swap(
                                map.mapObjects[collisionPos.first][collisionPos.second]);
        ...
        for (int i = 0; i < map.dragons.size(); i++) {
            if (map.dragons[i] != nullptr) {
                ...
                switch (flag) {
                    case CanMove: {
                        map.mapObjects[dragonPos.first][dragonPos.second].swap(
                                map.mapObjects[collisionObjectPos.first][collisionObjectPos.second]);
        ...
        if (userInput != ' ') {
            ...
            switch (userInput) {
                ...
                default:
                    ...
                    switch (flag) {
                        case CanMove:
                            map.mapObjects[knightPos.first][knightPos.second].swap(
                                    map.mapObjects[nextKnightPos.first][nextKnightPos.second]);
